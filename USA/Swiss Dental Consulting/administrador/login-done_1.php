        ...
        if($viBot == ""){
          ...
          if ($result = $connection->query($sqlCmd)) {
                  ...
                  if($enc->compareStrings($viPass,$decryptedData) == 'true'){
                    setcookie("usr_ck_name",$row['userName'],time()+86400);
                    setcookie("usr_ck_user",$row['keyuser'],time()+86400);
                    setcookie("usr_id",$row['id'],time()+86400);
                    setcookie("usr_per",$row['idTbPermissions'],time()+86400);
