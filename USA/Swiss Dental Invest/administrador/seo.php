  ...
  <!-- ./wrapper -->
    ...
    <script>
      function funCreateList(vfMainTable){
          ...
          xmlhttp.open("GET", "includes/seo.php?cmdEval=getSEO", true);
          xmlhttp.send();
      }
      ...
      function funDeleteItem(vfID){
          if (confirm('Tem a certeza que pertende eliminar o SEO?')) {
              var xmlhttp = new XMLHttpRequest();
              xmlhttp.onreadystatechange = function() {
                  if (this.readyState == 4 && this.status == 200) {
                      var response = this.responseText;
              ...
              xmlhttp.open("GET", "includes/seo.php?cmdEval=deleteSEO&id=" + vfID, true);
              xmlhttp.send();
          }
      }
