  ...
  <!-- ./wrapper -->
    ...
    <script>
      function funCreateList(vfMainTable){
          ...
          xmlhttp.open("GET", "includes/getInformation.php?cmdEval=getModel", true);
          xmlhttp.send();
      }
      ...
      function funDeleteItem(vfID){
          if (confirm('Tem a certeza que pertende eliminar este modelo')) {
              var xmlhttp = new XMLHttpRequest();
              xmlhttp.onreadystatechange = function() {
                  if (this.readyState == 4 && this.status == 200) {
                      var response = this.responseText;
              ...
              xmlhttp.open("GET", "includes/deleteInformation.php?cmdEval=deleteModelos&id=" + vfID, true);
              xmlhttp.send();
          }
      }
