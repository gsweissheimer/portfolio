    ...
    <script>
          ...
          xmlhttp.onreadystatechange = function() {
              if (this.readyState == 4 && this.status == 200) {
                  ...
                  for (var i = 0; i < response.length; i++) {
                    vfMainTable.row.add( [ response[i][0], response[i][1], response[i][3], response[i][2], response[i][4]])
                  }
                  vfMainTable.draw()
              }
          };
          var query = window.location.search.substring(1);
          xmlhttp.open("GET", "includes/packages.php?cmdEval=getPackage", true);
          xmlhttp.send();
      }
      ...
      function funDeleteItem(vfID){
          if (confirm('Sure that you want to delete this item?')) {
              ...
              xmlhttp.open("GET", "includes/packages.php?cmdEval=deletePackage&id=" + vfID, true);
              xmlhttp.send();
          }
      }
