  ...
  <!-- ./wrapper -->
    ...
    <script>
      function funCreateList(vfMainTable){
          ...
          xmlhttp.open("GET", "includes/country.php?cmdEval=getCountries", true);
          xmlhttp.send();
      }
      ...
      function funDeleteItem(vfID){
          if (confirm('Tem a certeza que pertende eliminar este País?')) {
              var xmlhttp = new XMLHttpRequest();
              xmlhttp.onreadystatechange = function() {
                  if (this.readyState == 4 && this.status == 200) {
                      var response = this.responseText;
              ...
              xmlhttp.open("GET", "includes/country.php?cmdEval=deleteCountry&id=" + vfID, true);
              xmlhttp.send();
          }
      }
