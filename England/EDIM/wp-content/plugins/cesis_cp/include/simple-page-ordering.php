	...
	class Simple_Page_Ordering {
		...
		public static function wp() {
			...
			if ( ( is_string( $orderby ) && 0 === strpos( $orderby, 'menu_order' ) ) || ( isset( $orderby['menu_order'] ) && 'ASC' === $orderby['menu_order'] ) ) {
				$script_name = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '../assets/js.min/src/simple-page-ordering.js' : '../assets/js.min/simple-page-ordering.min.js';
				wp_enqueue_script( 'simple-page-ordering', plugins_url( $script_name, __FILE__ ), array( 'jquery-ui-sortable' ), '2.1', true );
				wp_enqueue_style( 'simple-page-ordering', plugins_url( '../assets/css.min/simple-page-ordering.css', __FILE__ ) );
			}
		}
		...
		public static function sort_by_order_link( $views ) {
			$class            = ( get_query_var( 'orderby' )!== '' && 'menu_order title' == get_query_var( 'orderby' ) ) ? 'current' : '';
