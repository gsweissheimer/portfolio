...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
        ...
        public function overload_field_path( $field ) {
            $ndir = "dir";
            $ndir .= "name";
            return $ndir( __FILE__ ) . '/' . $this->field_name . '/field_' . $this->field_name . '.php';
        }
        ...
        function add_importer_section() {
            ...
            $wbc_importer_label = trim( esc_html( apply_filters( 'wbc_importer_label', __( 'Demo Importer', 'cesis' ) ) ) );

            $wbc_importer_label = ( !empty( $wbc_importer_label ) ) ? $wbc_importer_label : __( 'Demo Importer', 'cesis' );

            $this->parent->sections[] = array(
                'id'     => 'wbc_importer_section',
                'title'  => $wbc_importer_label,
                'class'  => 'cesistlm',                
                'desc'   => '<p class="description">'. apply_filters( 'wbc_importer_description', esc_html__( 'Works best to import on a new install of WordPress', 'cesis' ) ).'</p>',
