...
class Contentools_Rest {
    ...
    public function authenticate() {
        ...
        return (($_POST['token'] && isset($options['token']) && $_POST['token'] == $options['token']) ? true : false);
    ...
    public function action_publish_post( $request = null ) {
        ...
        if ( isset( $_POST["id"] ) ) $post['ID'] = $_POST['id'];
        if ( isset( $_POST["title"] ) ) $post['post_title'] = $_POST['title'];
        if ( isset( $_POST["author"] ) ) $post['post_author'] = $_POST['author'];
        if ( isset( $_POST["content"] ) ) $post['post_content'] = $_POST['content'];
        if ( isset( $_POST["excerpt"] ) ) $post['post_excerpt'] = $_POST['excerpt'];
        if ( isset( $_POST["slug"] ) ) $post['post_name'] = $_POST['slug'];
        if ( isset( $_POST["type"] ) ) $post['post_type'] = $_POST['type'];
        if ( isset( $_POST["status"] ) ) $post['post_status'] = $_POST['status'];
        if ( isset( $_POST["meta"] ) ) $post['meta_input'] = $_POST['meta'];
        if ( isset( $_POST["categories"] ) ) $post['post_category'] = $_POST['categories'];
        if ( isset( $_POST["tags"] ) ) $post['tags_input'] = $_POST['tags'];
        if ( isset( $_POST["featured_media"] ) ) $post['featured_media'] = $_POST['featured_media'];

        $id = !$post['ID'] ? wp_insert_post( wp_slash( $post ), true ) : wp_update_post( wp_slash( $post ), true );

        if ( !is_wp_error( $id ) ) {

            if ( isset( $_POST["featured_media"] ) ) update_post_meta( $id, '_thumbnail_id', $_POST['featured_media'] );
    ...
    public function action_find_post( $request = null ) {
        ...
        if ( is_numeric( $_POST['id'] ) && $post = get_post( $_POST['id'] ) ) {
    ...
    public function action_list_posts( $request = null ) {
        ...
        if ( $posts ) foreach ( $posts as $id => $post ) {
    ...
    public function action_publish_media( $request = null ) {
        ...
        if ( isset( $_FILES['file'] ) ) {
            ...
            $tempnam = wp_tempnam( $_POST['file'][0] );
        ...
        $attachment = array( 'file' => $file['file'], 'post_mime_type' => $file['type'], 'guid' => $file['url'], 'post_type' => 'attachment', 'post_status' => 'inherit' );

        if ( isset( $_POST["slug"] ) ) $attachment['post_name'] = $_POST['slug'];
        if ( isset( $_POST["title"] ) ) $attachment['post_title'] = $_POST['title'];
        elseif ( ! empty( $image_meta['title'] ) && trim( $image_meta['title'] ) ) $attachment['post_title'] = $image_meta['title'];
        else $attachment['post_title'] = preg_replace( '/\.[^.]+$/', '', basename( $attachment['file'] ) );        
        if ( isset( $_POST["author"] ) ) $attachment['post_author'] = $_POST['author'];
        if ( isset( $_POST["meta"] ) ) $attachment['meta_input'] = $_POST['meta'];
        if ( isset( $_POST["template"] ) ) $attachment['page_template'] = $_POST['template'];
        if ( isset( $_POST["description"] ) ) $attachment['post_content'] = $_POST['description'];
        if ( isset( $_POST["caption"] ) ) $attachment['post_excerpt'] = $_POST['caption'];
        elseif ( ! empty( $image_meta['caption'] ) && trim( $image_meta['caption'] ) ) $attachment['post_excerpt'] = $image_meta['caption'];
        if ( isset( $_POST["post"] ) ) $attachment['post_parent'] = $_POST['post'];
        ...
        if ( !is_wp_error( $id ) ) {
            ...
            if ( isset( $_POST['alt_text'] ) ) update_post_meta( $id, '_wp_attachment_image_alt', sanitize_text_field( $_POST['alt_text'] ) );
