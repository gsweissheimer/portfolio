...
if ( ! class_exists('scssc')) {
	class scssc {
		...
		protected
		function isSelfExtend( $target, $origin ) {
		...
		protected
		function pushExtends( $target, $origin ) {
		...
		protected
		function matchExtendsSingle( $single, &$outOrigin ) {
			$counts = array();
		...
		protected
		function combineSelectorSingle( $base, $other ) {
			$tag = null;
			$out = array();
		...
		protected
		function matchExtends( $selector, &$out, $from = 0, $initial = true ) {
		...
		protected
		function flattenSelectors( $block, $parentKey = null ) {
			if ( $block->selectors ) {
				$selectors = array();
		...
		protected
		function compileNestedBlock( $block, $selectors ) {
			$this->pushEnv( $block );
		...
		protected
		function flattenSelectorSingle( $single ) {
			$joined = array();
			foreach ( $single as $part ) {
		...
		protected
		function evalSelectorPart( $piece ) {
			foreach ( $piece as &$p ) {
				if ( ! is_array( $p ) ) {
					continue;
				}
		...
		protected
		function compileSelectorPart( $piece ) {
			foreach ( $piece as &$p ) {
				if ( ! is_array( $p ) ) {
					continue;
				}
		...
		protected
		function hasSelectorPlaceholder( $selector ) {
			if ( ! is_array( $selector ) ) {
				return false;
			}
		...
		protected
		function compileChildren( $stms, $out ) {
			foreach ( $stms as $stm ) {
		...
		protected
		function compileMediaQuery( $queryList ) {
			$out   = "@media";
			$first = true;
		...
		protected
		function mergeMediaTypes( $type1, $type2 ) {
			if ( empty( $type1 ) ) {
				return $type2;
			}
			if ( empty( $type2 ) ) {
		...
		protected
		function compileImport( $rawPath, $out ) {
			if ( $rawPath[0] == "string" ) {
		...
		protected
		function compileChild( $child, $out ) {
		...
		protected
		function reduce( $value, $inExp = false ) {
			list( $type ) = $value;
			switch ( $type ) {
				case "exp":
					...
					if ( is_callable( array(
						                  $this,
						                  $fn
					                  ) ) || ( ( $fn = "op_${ltype}_${rtype}" ) && is_callable( array(
						                                                                            $this,
						                                                                            $fn
					                                                                            ) ) && $passOp = true ) || ( ( $fn = "op_${opName}" ) && is_callable( array(
						                                                                                                                                                  $this,
						                                                                                                                                                  $fn
					                                                                                                                                                  ) ) && $genOp = true )
					) {
		...
		protected
		function coerceUnit( $number, $unit ) {
		...
		protected
		function op_add_number_number( $left, $right ) {
			return array( "number", $left[1] + $right[1], $left[2] );
		}

		protected
		function op_mul_number_number( $left, $right ) {
			return array( "number", $left[1] * $right[1], $left[2] );
		}

		protected
		function op_sub_number_number( $left, $right ) {
			return array( "number", $left[1] - $right[1], $left[2] );
		}

		protected
		function op_div_number_number( $left, $right ) {
			return array( "number", $left[1] / $right[1], $left[2] );
		}

		protected
		function op_mod_number_number( $left, $right ) {
			return array( "number", $left[1] % $right[1], $left[2] );
		}

		// adding strings
		protected
		function op_add( $left, $right ) {
		...
		protected
		function op_and( $left, $right, $shouldEval ) {
			if ( ! $shouldEval ) {
				return;
			}
			if ( $left != self::$false ) {
		...
		protected
		function op_or( $left, $right, $shouldEval ) {
			if ( ! $shouldEval ) {
				return;
			}
			if ( $left != self::$false ) {
		...
		protected
		function op_color_number( $op, $left, $right ) {
			$value = $right[1];
		...
		protected
		function op_number_color( $op, $left, $right ) {
			$value = $left[1];
		...
		protected
		function op_neq( $left, $right ) {
			return $this->toBool( $left != $right );
		}

		protected
		function op_gte_number_number( $left, $right ) {
			return $this->toBool( $left[1] >= $right[1] );
		}

		protected
		function op_gt_number_number( $left, $right ) {
			return $this->toBool( $left[1] > $right[1] );
		}

		protected
		function op_lte_number_number( $left, $right ) {
			return $this->toBool( $left[1] <= $right[1] );
		}

		protected
		function op_lt_number_number( $left, $right ) {
			return $this->toBool( $left[1] < $right[1] );
		}
		...
		protected
		function compileStringContent( $string ) {
			$parts = array();
		...
		protected
		function extractInterpolation( $list ) {
			$items = $list[2];
		...
		protected
		function multiplySelectors( $env ) {
			$envs = array();
			while( null !== $env ) {
		...
		protected
		function multiplyMedia( $env, $childQueries = null ) {
		...
		protected
		function coerceList( $item, $delim = "," ) {
		...
		protected
		function applyArguments( $argDef, $argValues ) {
			$hasVariable = false;
			$args        = array();
		...
		protected
		function getStoreEnv() {
			return isset( $this->storeEnv ) ? $this->storeEnv : $this->env;
		}

		protected
		function set( $name, $value, $shadow = false ) {
		...
		protected
		function setExisting( $name, $value, $env = null ) {
			if ( ! isset( $env ) ) {
		...
		public
		function get( $name, $defaultValue = null, $env = null ) {
			$name = $this->normalizeName( $name );
		...
		protected
		function injectVariables( array $args ) {
			if ( empty( $args ) ) {
				return;
			}
		...
		public
		function setImportPaths( $path ) {
			$this->importPaths = (array) $path;
		}

		public
		function setNumberPrecision( $numberPrecision ) {
			$this->numberPrecision = $numberPrecision;
		}

		public
		function setFormatter( $formatterName ) {
		...
		protected
		function callBuiltin( $name, $args, &$returnValue ) {
			// try a lib function
			$name    = $this->normalizeName( $name );
			$libName = "lib_" . $name;
		...
		protected
		function sortArgs( $prototype, $args ) {
			$keyArgs = array();
			$posArgs = array();
		...
		protected
		function coerceForExpression( $value ) {
		...
		protected
		function adjust_color_helper( $base, $alter, $i ) {
			return $base += $alter;
		}

		protected
		function lib_adjust_color( $args ) {
		...
		protected
		function change_color_helper( $base, $alter, $i ) {
			return $alter;
		}

		protected
		function lib_change_color( $args ) {
		...
		protected
		function scale_color_helper( $base, $scale, $i ) {
			// 1,2,3 - rgb
			// 4, 5, 6 - hsl
			// 7 - a
			switch ( $i ) {
		...
		protected
		function lib_transparentize( $args ) {
			$color  = $this->assertColor( $args[0] );
			$amount = $this->coercePercent( $args[1] );
		...
		protected
		function getNormalizedNumbers( $args ) {
			$unit         = null;
			$originalUnit = null;
			$numbers      = array();
		...
		protected
		function listSeparatorForJoin( $list1, $sep ) {
			if ( ! isset( $sep ) ) {
				return $list1[1];
			}
	...
	class scss_parser {
		...
		static protected
		function makeOperatorStr( $operators ) {
		...
		protected
		function pushSpecialBlock( $type ) {
		...
		protected
		function append( $statement, $pos = null ) {
			if ( $pos !== null ) {
				$statement[ - 1 ] = $pos;
				if ( ! $this->rootParser ) {
					$statement[ - 2 ] = $this;
		...
		protected
		function mediaQueryList( &$out ) {
			return $this->genericList( $out, "mediaQuery", ",", false );
		}

		protected
		function mediaQuery( &$out ) {
			$s = $this->seek();
		...
		protected
		function spaceList( &$out ) {
			return $this->genericList( $out, 'expression' );
		}

		protected
		function genericList( &$out, $parseItem, $delim = "", $flatten = true ) {
			$s     = $this->seek();
			$items = array();
		...
		protected
		function openString( $end, &$out, $nestingOpen = null ) {
			$oldWhite              = $this->eatWhiteDefault;
			$this->eatWhiteDefault = false;
		...
		protected
		function interpolation( &$out, $lookWhite = true ) {
			$oldWhite              = $this->eatWhiteDefault;
			$this->eatWhiteDefault = true;
		...
		protected
		function keyword( &$word, $eatWhitespace = null ) {
		...
		public
		function throwParseError( $msg = "parse error", $count = null ) {
		...
		protected
		function peek( $regex, &$out, $from = null ) {
			if ( ! isset( $from ) ) {
				$from = $this->count;
			}
	...
	class scss_formatter_nested extends scss_formatter {
		...
		public
		function adjustAllChildren( $block ) {
	...
	class scss_server {
		 ...
		 */
		protected
		function getModifiedSinceHeader() {
			$modifiedSince = '';
		 ...
		 */
		public
		function __construct( $dir, $cacheDir = null, $scss = null ) {
			$this->dir = $dir;
