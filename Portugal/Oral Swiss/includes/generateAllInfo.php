...
define ('URLAMIGAVEL', 'https://oralswiss.com/');//http://oralswiss.com/

/*echo '<pre>';
echo var_dump($_COOKIE['lang']);
echo '</pre>';
exit(0);*/
...
, $slideIndicators, $itemMainSlide, $classToAdd = 'active', $classIfNotFirstToAdd = '', $arr = null)
{
    ...
    $sqlCmdGetInfo = "SELECT
                    ...
                    ORDER BY tb_slide_translation.id
                    ";
    ...
    if ($result = $connection->query($sqlCmdGetInfo)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $eleSecTitle = str_ireplace('{{subtitle}}', strip_tags($row['subtitle'], $allowednobr), $eleSecTitle);
            $eleSecTitle = str_ireplace('{{title}}', strip_tags($row['title'], $allowed), $eleSecTitle);
            $eleSecTitle = str_ireplace('{{pos}}', $rpos, $eleSecTitle);
            $eleSecTitle = str_ireplace('{{posSlide}}', $i, $eleSecTitle);
            $eleSecTitle = str_ireplace('{{text}}', strip_tags($row['text'], $allowed), $eleSecTitle);
            $eleSecTitle = str_ireplace('{{active}}', $active, $eleSecTitle);
            $eleSecTitle = str_ireplace('{{description}}', strip_tags($row['text'], $allowed), $eleSecTitle);
            ...
            $eleItemMainSlide = str_ireplace('{{subtitle}}', strip_tags($row['subtitle'], $allowednobr), $eleItemMainSlide);
...
function funGetSlideMob($slideCode, $secTitle
, $slideIndicators, $itemMainSlide, $classToAdd = 'active', $classIfNotFirstToAdd = '', $arr = null)
{
    ...
    if ($result = $connection->query($sqlCmdGetInfo)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $mob_img = explode("/", $mob_img);

            $mob_img = URLAMIGAVEL . "assets/gallery/images/mob-" . end($mob_img);
            ...
            $eleItemMainSlide = str_ireplace('{{active}}', $active, $eleItemMainSlide);
            $eleItemMainSlide = str_ireplace('{{img}}', URLAMIGAVEL . $row['path'], $eleItemMainSlide);
            $eleItemMainSlide = str_ireplace('{{imgg}}', $mob_img, $eleItemMainSlide);
            $eleItemMainSlide = str_ireplace('{{alt}}', $row['alt'], $eleItemMainSlide);
            $eleItemMainSlide = str_ireplace('{{title}}', $row['title'], $eleItemMainSlide);
            $eleItemMainSlide = str_ireplace('{{posSlide}}', $i, $eleItemMainSlide);
            $eleItemMainSlide = str_ireplace('{{description}}', $row['text'], $eleItemMainSlide);
...
function funGetMenu($tag, $html, $children_html = null, $children_html_pre = '', $children_html_pos = '', $children = null, $extra= null)
{
    if (isset($_COOKIE['lang'])) {
        $lang = $_COOKIE['lang'];
    } else {
    ...
    foreach ($menu as $i => $item) {
        ...
        $elem = str_ireplace('{{url}}', $item['url'], $elem);
...
function funGetAdvancedBannersAsArray($tag, $country = 1, $lang = 'PT')
{
  ...
  ORDER BY tb_advanced_banner.position";
    ...
    if ($result = $connection->query($sql)) {
        while ($row = mysqli_fetch_assoc($result)) {
            $data = array(
                'title' => strip_tags($row['title'],'<br><b>'),
                'subtitle' => strip_tags($row['subtitle'],'<br><b>'),
                'text' => strip_tags($row['text'],'<br><b>'),
                'subtext' => strip_tags($row['subText'],'<br><b>'),
...
function funGetAdvancedBanners($tag, $html, $strip_tags = true, $search = "")
{
    ...
    foreach ($banners as $i => $banner) {
        ...
        $item = str_ireplace('{{position}}', $banner['position'], $item);
        $item = str_ireplace('{{duration}}', $banner['duration'], $item);
