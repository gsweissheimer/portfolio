  ...
  <!-- ./wrapper -->
    ...
    <script>
      function funCreateList(vfMainTable){
          ...
          xmlhttp.open("GET", "includes/news.php?cmdEval=getNews", true);
          xmlhttp.send();
      }
      ...
      function funDeleteItem(vfID){
          if (confirm('Tem a certeza que pertende eliminar esta notícia')) {
              var xmlhttp = new XMLHttpRequest();
              xmlhttp.onreadystatechange = function() {
                  if (this.readyState == 4 && this.status == 200) {
                      var response = this.responseText;
              ...
              xmlhttp.open("GET", "includes/news.php?cmdEval=deleteNews&id=" + vfID, true);
              xmlhttp.send();
          }
      }
